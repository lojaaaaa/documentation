...
git merge feature
```
👆 Создание нового коммита, в котором обьеденены коммиты из двух веток
...
🎯 `git reset --hard`   
&emsp;&emsp; 👆 Откинет на состояние ветки, до попытки слияния, но сотрет все незакомиченные изменения в `workDirectory`  

🎯 `git reset --merge`   
&emsp;&emsp; 👆 Откинет на состояние ветки, до попытки слияния, но сохранит все незакомиченные изменения, которые не учавствовали в слиянии
